...
class _CatererDetailPageState extends ConsumerState<CatererDetailPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: catererAsync.when(
        data: (caterer) {
          final packagesAsync = ref.watch(packagesForProviderProvider(caterer.userId));
          return SafeArea(
        ...
        background: Stack(
          ...
          children: [
            ...
            PageView.builder(
              ...
              itemCount: caterer.menuItems.isEmpty ? 1 : caterer.menuItems.length,
              onPageChanged: (index) {
                setState(() {
                ...
                return Container(
                  decoration: BoxDecoration(
                    image: DecorationImage(
                      image: NetworkImage(caterer.menuItems[index]),
                      fit: BoxFit.cover,
                    ),
                  ),
                );
            ...
            // Image indicators
            if (caterer.menuItems.isNotEmpty)
              Positioned(
                bottom: 20,
                left: 0,
                right: 0,
                child: Row(
                  ...
                  children: List.generate(caterer.menuItems.length, (index) {
  ...
  Future<void> _handleBookingSubmission(WidgetRef ref, ThemeData theme) async {
    try {
      ...
      final packagesAsync = ref.read(packagesForProviderProvider(caterer?.userId ?? ''));
